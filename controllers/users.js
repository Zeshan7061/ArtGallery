...
module.exports = {
            ...
            bcrypt.genSalt(10, (err, salt) => {
              bcrypt.hash(user.password, salt, (err, hash) => {
                ...
                user.save().then(async (savedUser) => {
                  req.flash(
                    'success_msg',
                    'We have sent a confirmation link to your email. Please verify your email.'
                  );
                  //   req.flash(
                  //     'success_msg',
                ...
                bcrypt.compare(
                  ...
                  async (err, matched) => {
                    ...
                    if (
                      user.blockTime &&
                      Math.floor(
                        (new Date().getTime() - user.blockTime) / 1000 < 300
                      )
                    ) {
                      return done(
                        null,
                        false,
                        "You can't login for 5 minutes due to many failed login attempts."
                      );
                    }
                    ...
                    if (matched) {
                      ...
                      if (user.status == 'Disabled') {
                        return done(null, false, {
                          message:
                            'You are temporarily blocked due to some reason!',
                        });
                      }
  ...
  updateAccountInfo: async (req, res, next) => {
    try {
      ...
      if (req.files) {
        if (
          user.image != 'user.jpg' &&
          fs.existsSync(`./public/uploads/user/${user.image}`)
        ) {
          ...
          bcrypt.genSalt(10, (err, salt) => {
            bcrypt.hash(req.body.password, salt, (err, hash) => {
              ...
              user.save().then((savedUser) => {
                req.flash(
                  'success_msg',
                  'Your password has been reset succesfully. Log in to continue'
                );
  ...
  placeBid: async (req, res, next) => {
    try {
      ...
      if (
        +req.body.bid_amount <= post.price ||
        +req.body.bid_amount <= post.bidInfo.amount
      ) {
  ...
  postComment: async (req, res, next) => {
    try {
      ...
      req.flash(
        'success_msg',
        'Your comment will be approved by admin before posting. Thanks'
      );
  ...
  contactAdmin: async (req, res, next) => {
    try {
      ...
      req.flash(
        'success_msg',
        'Your message has been sent to the admin. Thanks'
      );
  ...
  readNotification: async (req, res, next) => {
    try {
      ...
      let url = '';
      url =
        req.params.url == 'Admin'
          ? '/admin/users/adminWarnings'
          : `/admin/inbox/chat/${n.roomId}/${n.username}`;
